...
export default function PhotographerDetail() {
  ...
  return (
    <div className="space-y-6">
      {/* Breadcrumb */}
      <Link href="/photographers" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 transition-colors">
        <ChevronLeft className="h-4 w-4" /> Back to Photographers
      </Link>

      {/* Hero card */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="h-28 bg-gradient-to-r from-indigo-600 via-violet-500 to-indigo-400" />
        <div className="px-6 pb-6 -mt-12">
          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
            <div className="flex items-end gap-4">
              ...
              />
              <div className="mb-1">
                <h1 className="text-2xl font-bold text-slate-900">{photographer.name}</h1>
                <p className="text-slate-500 mt-0.5">{photographer.specialization}</p>
            ...
            <div className="flex gap-2">
              <button className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-all">
                <Edit className="h-4 w-4" /> Edit Profile
              </button>
              <button className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium transition-all shadow-sm">
                Update Availability
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 text-sm">
                ...
                <a href={`mailto:${photographer.email}`} className="text-indigo-600 hover:underline truncate">{photographer.email}</a>
              </div>
              <div className="flex items-center gap-3 text-sm">
                <Phone className="h-4 w-4 text-slate-400 shrink-0" />
                <span className="text-slate-700">{photographer.phone}</span>
              </div>
              <div className="flex items-center gap-3 text-sm">
                <MapPin className="h-4 w-4 text-slate-400 shrink-0" />
                <span className="text-slate-700">{photographer.city}</span>
              </div>
            </div>
          </div>
          ...
          {/* Skills */}
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <h3 className="text-sm font-bold text-slate-900 mb-3">Specializations</h3>
            <div className="flex flex-wrap gap-2">
              {(photographer.skills?.length ? photographer.skills : fake.skills).map((skill) => (
                <span key={skill} className="px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-medium border border-indigo-100">
                  {skill}
                </span>
              ))}
            </div>
          ...
          {/* Working days */}
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <h3 className="text-sm font-bold text-slate-900 mb-3">Working Days</h3>
            <div className="flex flex-wrap gap-2">
              {(photographer.workingDays?.length ? photographer.workingDays : fake.workingDays).map((day) => (
                <span key={day} className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 text-xs font-medium">
                  {day}
                </span>
              ))}
            </div>
          ...
          {(photographer.bio || fake.bio) && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <p className="text-sm text-slate-600 leading-relaxed">{photographer.bio || fake.bio}</p>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {portfolioImages.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <h3 className="text-sm font-bold text-slate-900 mb-4">Portfolio</h3>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-slate-50">
              {!(photographer.upcomingBookings || fake.upcomingBookings)?.length ? (
                <p className="px-5 py-6 text-sm text-slate-400 text-center">No upcoming bookings.</p>
              ) : (
                (photographer.upcomingBookings || fake.upcomingBookings).map((b) => (
                  <div key={b.id} className="flex items-center gap-4 px-5 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-900">{b.clientName}</p>
                      <p className="text-xs text-slate-400 capitalize">{b.eventType} · {b.location}</p>
                    </div>
                    <span className="text-xs text-slate-500 shrink-0">{format(new Date(b.date), "MMM d, yyyy")}</span>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-900">Completed Shoots</h3>
            </div>
            <div className="divide-y divide-slate-50">
              {!(photographer.completedShoots || fake.completedShoots)?.length ? (
                <p className="px-5 py-6 text-sm text-slate-400 text-center">No completed shoots.</p>
              ) : (
                (photographer.completedShoots || fake.completedShoots).map((b) => (
                  <div key={b.id} className="flex items-center gap-4 px-5 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-900">{b.clientName}</p>
                      <p className="text-xs text-slate-400 capitalize">{b.eventType}</p>
                    </div>
