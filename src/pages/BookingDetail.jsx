...
export default function BookingDetail() {
  ...
  const confirmAssignment = () => {
    ...
    if (apiBooking) {
      ...
      const day = FAKE_BOOKINGS[id]?.days.find(d => d.dayNumber === selectedDay.dayNumber);
      if (day) day.photographerName = assigningPhotographer.name;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <Link href="/bookings" className="inline-flex items-center gap-1 text-sm text-slate-500 dark:text-muted-foreground hover:text-slate-700 dark:text-muted-foreground">
            <ChevronLeft className="h-4 w-4" /> Back
          </Link>
          <div className="w-px h-5 bg-slate-200" />
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-xl font-bold text-slate-900 dark:text-foreground">{booking.clientName}</h1>
              <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${sc.cls}`}>{sc.label}</span>
            </div>
            <p className="text-sm text-slate-500 dark:text-muted-foreground mt-0.5 capitalize">{booking.eventType} · Booking #{booking.id}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white dark:bg-card rounded-2xl border border-slate-100 dark:border-border shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
              {[
                ...
                <div key={label}>
                  ...
                  <p className={`text-sm font-semibold text-slate-800 dark:text-foreground ${capitalize ? "capitalize" : ""}`}>
                    {Icon && <Icon className="inline h-3.5 w-3.5 text-slate-400 dark:text-muted-foreground/70 mr-1" />}
                    {value}
                  </p>
                </div>
              ))}
            ...
            {booking.notes && (
              <div className="mt-5 pt-5 border-t border-slate-100 dark:border-border">
                <p className="text-xs font-medium text-slate-400 dark:text-muted-foreground/70 uppercase tracking-wide mb-2">Client Notes</p>
                <p className="text-sm text-slate-600 dark:text-muted-foreground bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 leading-relaxed">{booking.notes}</p>
              </div>
            )}
          </div>
          ...
          {/* Days & assignments */}
          <div className="bg-white dark:bg-card rounded-2xl border border-slate-100 dark:border-border shadow-sm p-6">
            <h3 className="text-sm font-bold text-slate-900 dark:text-foreground mb-4">Event Days & Photographer Assignment</h3>
            <div className="space-y-3">
              {booking.days?.map((day) => (
                <div key={day.dayNumber} className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-border">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <div className="h-6 w-6 rounded-full bg-primary/20 flex items-center justify-center text-[11px] font-bold text-primary">
                        {day.dayNumber}
                      </div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-muted-foreground ml-8">{day.details}</p>
                  ...
                  <button
                    ...
                    className={`shrink-0 px-3.5 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                      day.photographerName
                        ? "border border-slate-200 dark:border-border/60 text-slate-600 dark:text-muted-foreground hover:bg-slate-100 dark:bg-slate-800"
          ...
          {selectedDay && (
            <div className="bg-white dark:bg-card rounded-2xl border-2 border-primary/20 shadow-sm p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-foreground">Select Photographer — Day {selectedDay.dayNumber}</h3>
                  <p className="text-xs text-slate-500 dark:text-muted-foreground mt-0.5">{format(new Date(selectedDay.date), "EEEE, MMMM d, yyyy")}</p>
                </div>
                <button onClick={() => setSelectedDay(null)} className="text-xs text-slate-400 dark:text-muted-foreground/70 hover:text-slate-600 dark:text-muted-foreground">Cancel</button>
              </div>
              ...
              {isAvailabilityLoading ? (
                ...
                <div className="space-y-5">
                  <div>
                    <p className="text-xs font-semibold text-slate-500 dark:text-muted-foreground uppercase tracking-wide mb-3">Available Photographers</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {availablePhotographers.map((photo) => (
                        <div key={photo.id} className="flex items-center gap-3 p-3.5 border border-slate-200 dark:border-border/60 rounded-xl bg-white dark:bg-card hover:border-primary/20 hover:bg-primary/10/40 transition-all">
                          <img
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-slate-900 dark:text-foreground">{photo.name}</p>
                            <div className="flex items-center gap-2">
                              <p className="text-xs text-slate-400 dark:text-muted-foreground/70 truncate">{photo.specialization}</p>
                              {photo.rating && (
                          ...
                          <button
                            ...
                            className="shrink-0 px-3 py-1.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-xs font-semibold transition-all"
                          >
                            Assign
                          </button>
                        </div>
                  ...
                  {unavailablePhotographers.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-slate-400 dark:text-muted-foreground/70 uppercase tracking-wide mb-3">Unavailable</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 opacity-60">
                        {unavailablePhotographers.map((photo) => (
                          <div key={photo.id} className="flex items-center gap-3 p-3.5 border border-slate-100 dark:border-border rounded-xl bg-slate-50 dark:bg-slate-900/50">
                            <img
                              src={photo.avatarUrl || `https://i.pravatar.cc/100?u=${photo.id}`}
                              alt={photo.name}
                            ...
                            />
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold text-slate-700 dark:text-muted-foreground">{photo.name}</p>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white dark:bg-card rounded-2xl border border-slate-100 dark:border-border shadow-sm p-5">
            <h3 className="text-sm font-bold text-slate-900 dark:text-foreground mb-4">Client Information</h3>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-slate-900 dark:text-foreground text-sm">{booking.clientName}</p>
                <p className="text-xs text-slate-400 dark:text-muted-foreground/70">Client</p>
              </div>
            </div>
            <div className="space-y-3">
              <a href={`mailto:${booking.clientEmail}`} className="flex items-center gap-2.5 text-sm text-primary hover:text-primary">
                <Mail className="h-4 w-4 shrink-0 text-slate-400 dark:text-muted-foreground/70" /> {booking.clientEmail}
              </a>
              <a href={`tel:${booking.clientPhone}`} className="flex items-center gap-2.5 text-sm text-slate-700 dark:text-muted-foreground">
                <Phone className="h-4 w-4 shrink-0 text-slate-400 dark:text-muted-foreground/70" /> {booking.clientPhone}
              </a>
              <div className="flex items-start gap-2.5 text-sm text-slate-600 dark:text-muted-foreground">
                <Calendar className="h-4 w-4 shrink-0 text-slate-400 dark:text-muted-foreground/70 mt-0.5" />
                {format(new Date(booking.eventDate), "MMMM d, yyyy")}
              </div>
              <div className="flex items-start gap-2.5 text-sm text-slate-600 dark:text-muted-foreground">
                <MapPin className="h-4 w-4 shrink-0 text-slate-400 dark:text-muted-foreground/70 mt-0.5" /> {booking.location}
              </div>
            </div>
          </div>
          ...
          {/* Financials */}
          <div className="bg-white dark:bg-card rounded-2xl border border-slate-100 dark:border-border shadow-sm p-5">
            <h3 className="text-sm font-bold text-slate-900 dark:text-foreground mb-4">Financials</h3>
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 dark:text-muted-foreground">Package</span>
                <span className="font-medium text-slate-800 dark:text-foreground text-right max-w-[140px]">{booking.packageName || "Custom"}</span>
              </div>
              ...
              <div className="pt-3 mt-3 border-t border-slate-100 dark:border-border flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-slate-900 dark:text-foreground">{formatCurrency(booking.totalPrice, settings.currency)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="pt-1">
              You're about to assign <span className="font-semibold text-slate-900 dark:text-foreground">{assigningPhotographer?.name || "this photographer"}</span> to{" "}
              <span className="font-semibold text-slate-900 dark:text-foreground">Day {selectedDay?.dayNumber || ""}</span> {selectedDay ? `(${format(new Date(selectedDay.date), "MMMM d, yyyy")})` : ""}.
          ...
          {assigningPhotographer && (
            <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-border/60">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-muted-foreground">{assigningPhotographer.specialization}</p>
              </div>
            </div>
          )}
